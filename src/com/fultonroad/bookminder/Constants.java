package com.fultonroad.bookminder;

public class Constants {

	public static final String kTAG_FRAGMENT_BOOK = "book_fragment";
	public static final String kTAG_FRAGMENT_BOOK_VIEW = "book_view_fragment";
	public static final String kTAG_FRAGMENT_AUTHOR = "author_fragment";

	
	public static final int kLOADER_ID_BOOK = 0;
	public static final int kLOADER_ID_BOOK_VIEW = 1;
	public static final int kLOADER_ID_AUTHOR = 2;

	
	
	public static final int kMSG_AUTHORS = 0;
	public static final int kMSG_BOOKS = 1;
	
	public static final int kMSG_REFRESH = 0;
	public static final int kMSG_DROP = 1;
	
	
}
